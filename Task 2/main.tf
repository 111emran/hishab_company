...
module "my_alb_asg" {
    ...
    user_data = <<-EOF
      ...
      mysql -h ${module.my_rds.host} -u ${module.my_rds.username} -p${var.db_password} < /var/www/html/hishab-db.sql
      sudo sed -i 's/##DB_HOST##/${module.my_rds.host}/' /var/www/html/hishab-db-config.sql
      sudo sed -i 's/##DB_USER##/${module.my_rds.username}/' /var/www/html/hishab-db-config.sql
      sudo sed -i 's/##DB_PASSWORD##/${var.db_password}/' /var/www/html/hishab-db-config.sql
    EOF  
}
